...
func (f *fakeOrderService) CreateOrder(order *entity.Order) error {
...
func (f *fakeOrderService) GetOrderByID(id string) (*entity.Order, error) {
...
func (f *fakeOrderService) DeleteOrder(id string) error {
...
func TestUpdateOrder(t *testing.T) {
	...
	reqUpdate, _ := http.NewRequest("PUT", "/orders/"+order.ID, bytes.NewBuffer(updateBody))
...
func TestDeleteOrder(t *testing.T) {
	...
	reqDelete, _ := http.NewRequest("DELETE", "/orders/"+order.ID, nil)
	wDelete := httptest.NewRecorder()
	router.ServeHTTP(wDelete, reqDelete)
	assert.Equal(t, http.StatusOK, wDelete.Code)

	reqGet, _ := http.NewRequest("GET", "/orders/"+order.ID, nil)
